...
class User(UserMixin, db.Model):
    ...
    message_sent: so.WriteOnlyMapped['Message'] = so.relationship(foreign_keys='Message.sender_id', back_populates='author')
    message_received: so.WriteOnlyMapped['Message'] = so.relationship(foreign_keys='Message.recipient_id', back_populates='recipient')
    ...
    def verify_reset_password_token(token):
        try: 
            id = jwt.encode(token, current_app.config['SECRET_KEY'],
    ...
    #Aplica logica para saber si la contraseña está en la base de datos
    ...
    def following_posts(self):
        ...
        return (
            sa.select(Post).join(Post.author.of_type(Author)).join(Author.followers.of_type(Follower), isouter=True).where(sa.or_(Follower.id == self.id, Author.id == self.id)).order_by(Post.timestamp.desc()).group_by(Post).order_by(Post.timestamp.desc())
                )
    def unread_message_count(self):
        last_read_time = self.last_message_read_time if isinstance(self.last_message_read_time, datetime) else datetime(1900, 1, 1)
...
class Message(db.Model):
    ...
    author: so.Mapped[User] = so.relationship(foreign_keys='Message.sender_id', back_populates='message_sent')
    recipient: so.Mapped[User] = so.relationship(foreign_keys='Message.recipient_id', back_populates='message_received')
